package com.mgn.ai.ui.components.ui
...
import com.mgn.ai.ui.hooks.useThrottle
import com.mgn.ai.ui.pages.chat.ChatVM
import com.mgn.ai.utils.UpdateDownload
import com.mgn.ai.utils.Version
import com.mgn.ai.ui.hooks.readStringPreference
import com.mgn.ai.ui.hooks.writeStringPreference
import com.mgn.ai.utils.onError
import com.mgn.ai.utils.onSuccess
import com.mgn.ai.utils.toLocalDateTime
...
fun UpdateCard(vm: ChatVM) {
    ...
    state.onSuccess { info ->
        ...
        var dismissedVersion by remember(info.version) {
            mutableStateOf(context.getDismissedUpdateVersion())
        }
        val current = remember { Version(BuildConfig.VERSION_NAME) }
        val latest = remember(info) { Version(info.version) }
        if (latest > current && dismissedVersion != info.version) {
            Card(
                onClick = {
                    showDetail = true
                }
                ...
                Column(
                    ...
                    Row(
                        ...
                        IconButton(onClick = {
                            context.setDismissedUpdateVersion(info.version)
...
private const val KEY_DISMISSED_UPDATE_VERSION = "update_dismissed_version"

private fun Context.getDismissedUpdateVersion(): String? =
    readStringPreference(KEY_DISMISSED_UPDATE_VERSION)

private fun Context.setDismissedUpdateVersion(version: String) =
    writeStringPreference(KEY_DISMISSED_UPDATE_VERSION, version)

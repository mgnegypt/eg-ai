package com.mgn.ai.workflow.execution
...
import com.mgn.ai.workflow.model.WorkflowDefinition
import com.mgn.ai.workflow.model.WorkflowRunStatus
import com.mgn.ai.workflow.repository.WorkflowRepository
import com.mgn.ai.workflow.trigger.TriggerFireCallback
...
class WorkflowEngine(
     ...
     */
    private val agentRunRepo: com.mgn.ai.data.agentrun.AgentRunRepository by lazy {
        org.koin.java.KoinJavaComponent.getKoin().get<com.mgn.ai.data.agentrun.AgentRunRepository>()
    }
     ...
     * [com.mgn.ai.workflow.repository.WorkflowRepository.deleteCascading] so the
    ...
    private suspend fun fireLocked(workflowId: String): FireOutcome {
        ...
        val ledgerId = agentRunRepo.open(
            kind = com.mgn.ai.data.agentrun.AgentRunKind.Workflow,
            domainId = workflowId,
     ...
     */
    private fun triggerRuntimeCheck(trigger: com.mgn.ai.workflow.model.TriggerSpec): String? {
        ...
        return when (trigger) {
            is com.mgn.ai.workflow.model.TriggerSpec.GeofenceEnter,
            is com.mgn.ai.workflow.model.TriggerSpec.GeofenceExit -> {
            ...
            is com.mgn.ai.workflow.model.TriggerSpec.BluetoothDeviceDisconnected -> {
    ...
    private suspend fun persistAndReturn(
        ...
        val ledgerStatus = when (status) {
            WorkflowRunStatus.SUCCESS -> com.mgn.ai.data.agentrun.AgentRunStatus.succeeded
            WorkflowRunStatus.FAILED -> com.mgn.ai.data.agentrun.AgentRunStatus.failed
            else -> com.mgn.ai.data.agentrun.AgentRunStatus.cancelled
        }
        agentRunRepo.markTerminal(
            id = ledgerId,
            status = ledgerStatus,
            lastError = error ?: if (ledgerStatus == com.mgn.ai.data.agentrun.AgentRunStatus.cancelled) status.name else null,
        )
...
/**
 * Sequential action runner — wraps [com.mgn.ai.service.DirectModeActionRunner]'s
...
class WorkflowActionRunner {
    ...
    suspend fun run(actions: List<WorkflowAction>, availableTools: List<Tool>): RunResult {
        ...
        for ((idx, action) in actions.withIndex()) {
            ...
            val text = out.filterIsInstance<com.mgn.ai.ai.ui.UIMessagePart.Text>()
                .joinToString("\n") { it.text }

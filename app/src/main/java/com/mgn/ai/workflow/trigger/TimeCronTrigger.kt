 ...
 * fire computed exactly via [com.mgn.ai.service.CronExpressionParser] (shared with
...
internal class TimeCronTriggerFamily(
    ...
    suspend fun onWorkerFired(workflowId: String) {
        ...
        val wf = lastSnapshot.firstOrNull { it.id == workflowId }
            ?: run {
                val loaded = com.mgn.ai.workflow.trigger.TimeCronWorkerHelper
    ...
    companion object {
        ...
        fun computeNextFireMs(spec: TriggerSpec.TimeCron, zone: ZoneId, nowMs: Long): Long {
            ...
            spec.cron?.trim()?.takeIf { it.isNotBlank() }?.let { cron ->
                com.mgn.ai.service.CronExpressionParser.parse(cron).getOrNull()?.let { parsed ->
                    com.mgn.ai.service.CronExpressionParser.nextExecution(parsed, now)
